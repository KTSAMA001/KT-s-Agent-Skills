...
在执行任何安装操作前，AI Agent **必须**按以下流程告知用户，并等待用户确认：

```mermaid
flowchart TB
    A[用户请求安装] --> B{是否已告知注意事项?}
    B -->|否| C[⚠️ 显示仓库现状说明]
    B -->|是| H[继续安装流程]

    C --> D[显示: main为混合分支<br/>通用经验与个人经验共存]
    D --> E[显示: 需要自行筛选适用性]
    E --> F[询问用户选择安装方式]

    F --> G{用户选择}
    G -->|直接使用| H[执行标准克隆]
    G -->|Fork| I[提示用户先Fork再克隆]
    G -->|选择性| J[提醒克隆后删除不需要的目录]
    G -->|暂不安装| K[结束流程]
...
> - 尚未进行分支隔离，所有经验都在同一个分支中
>
> **注意事项**：
> - 仓库中的经验不一定适用于你的项目，请根据实际情况判断
> - 个人/项目特定内容（如特定配置、代码风格等）仅供参考
...
| 用户选择 | AI Agent 操作 |
|---------|-------------|
| 直接使用 | 执行 `git clone`，并提醒安装后自行筛选 |
| Fork | 指导用户在 GitHub 上 Fork，然后克隆用户的仓库 |
| 选择性 | 执行克隆，并列出可删除的目录示例 |
| 暂不安装 | 取消安装流程，询问是否需要其他帮助 |
