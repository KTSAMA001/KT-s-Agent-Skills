...
- [2026-01-29] 王燚提交验证，变体优化后编译时间和内存显著下降

**备注**：

此经验来源于项目实际开发中王燚的 Shader 重构工作，适用于使用 ASE + Bakery + URP 的项目。
...
- 将复杂功能（如 SH 光照、RimLight、LatLong 特效）拆分为独立的 ASE 节点组
- 每个节点组负责单一功能，对外暴露必要参数
- 主 Shader 图只负责组合各节点组，而非实现具体逻辑

### 2. SH 光照参数外置

让节点可以灵活控制光照来源，提高通用性。将 L0 从外部节点传入而非内部计算。

### 3. Include 路径使用相对路径

使用相对路径而非绝对路径引用 hlsl 文件，避免项目迁移时路径失效。

### 4. 移除不必要的 multi_compile

减少 Shader 变体数量（可从十万级降至百级），显著降低编译时间和内存占用。

### 5. 资源清理

定期清理废弃的材质和 Shader 资源，保持项目结构清晰。
