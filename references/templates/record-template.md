...
> Web 渲染管线从此表自动解析字段定义，新增/改名字段无需改动前端代码。
...
> Web 渲染管线从此表解析状态 Emoji → SVG/颜色映射。
...
> 源文件保留 Emoji 以保持 raw markdown 的人类可读性。Web 渲染管线在 sync 阶段自动将 Emoji 转换为对应 SVG 图标。新增 Emoji 使用前须在本表注册映射关系。

| Emoji | SVG 图标 | CSS 类 | 说明 |
|-------|----------|--------|------|
| ✅ | mark-check | mark-check | 推荐/正确标记（正文中） |
| ❌ | mark-cross | mark-cross | 避免/错误标记（正文中） |
| ⚠️ | status-pending | mark-warning | 注意/警告标记（正文中） |
| 🟢 | indicator-green | indicator--green | 等级指示：低/好 |
| 🟡 | indicator-yellow | indicator--yellow | 等级指示：中 |
| 🟠 | indicator-orange | indicator--orange | 等级指示：较高 |
| 🔴 | indicator-red | indicator--red | 等级指示：高/差 |
